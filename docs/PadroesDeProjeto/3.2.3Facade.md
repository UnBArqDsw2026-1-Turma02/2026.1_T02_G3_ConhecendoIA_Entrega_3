...
Classificado como um padrão de projeto **estrutural**, o Facade (ou Fachada) tem como principal objetivo disponibilizar uma interface limpa, unificada e de alto nível para ocultar a complexidade de um ou mais subsistemas. Dessa forma, ele atua como um "escudo" que poupa o cliente de lidar com as particularidades de classes internas, entregando uma forma de uso muito mais acessível.
...
O Facade surge justamente para mitigar esse caos. Ele centraliza as invocações complexas em métodos de fácil compreensão, assumindo a responsabilidade de repassar as ordens aos subsistemas apropriados reduzindo a necessidade de interação direta com as classes internas do sistema.
...
* **Facade (A Fachada):**
    ...
    * Importante: a fachada não deve abrigar lógica de negócio primária. Ela apenas orquestra e delega chamadas.
...
Nesta seção, detalharemos a implementação do Facade dentro do projeto **ConhecendoIA**.
...
>
> 2. **Link da implementação do Facade:** [Código - facade.js](https://github.com/UnBArqDsw2026-1-Turma02/2026.1_T02_G3_ConhecendoIA_Entrega_3/blob/main/frontend/src/lib/facade.js)

No projeto, o arquivo `facade.js` atua como uma **Fachada** para centralizar as interações com os variados serviços da plataforma, como gerenciar trilhas de aprendizagem, progresso, temas e usuários.

#### 2.1.1. Contexto do Problema

A aplicação depende de diversas classes de serviço, que manipulam diferentes áreas:
- **`TrilhaService`:** Lida com a criação e montagem de trilhas (utilizando o padrão Builder).
- **`UsuarioService`:** Cadastra e gerencia o armazenamento de usuários em memória.
- **`ProgressoService`:** Inicia, registra avanços e busca o progresso de usuários (utilizando o padrão Observer).
- **`TemaService`:** Cadastra e lista temas de inteligência artificial.
...
O Facade resolve isso encapsulando essas classes em `PlataformaFacade`.

#### 2.1.2. A Fachada em Ação (`facade.js`)

A `PlataformaFacade` centraliza as instâncias dos serviços em seu construtor e provê métodos muito diretos para que o cliente realize as ações, delegando as requisições para o respectivo serviço sob o capô:
...
```javascript
// O cliente conhece APENAS a PlataformaFacade
const plataforma = new PlataformaFacade();

// Usando o subsistema de usuários de forma transparente
const arthur = plataforma.cadastrarUsuario(1, "Arthur", "arthur@email.com", "hash123");

// Usando o subsistema de trilhas
...
Abaixo o diagrama de classes que representa o uso da Fachada na arquitetura central da plataforma ConhecendoIA:
...
- O **Client** (que representa o ponto de uso principal ou interface) não se comunica diretamente com os múltiplos serviços de gestão do sistema. Ele interage exclusivamente com a **PlataformaFacade**.
- A **PlataformaFacade** atua como a fachada do sistema, expondo métodos unificados e simplificados (como `cadastrarUsuario()` ou `criarTrilha()`).
- Por baixo dos panos, a fachada detém as referências e delega as chamadas para as classes especialistas do subsistema: **TrilhaService**, **UsuarioService**, **ProgressoService** e **TemaService**.
- Dessa forma, o Cliente fica isolado da complexidade interna e das regras de negócio (como o uso de Builders e Observers dentro dos serviços), precisando conhecer apenas uma única interface para orquestrar todas as operações.

#### 2.1.4. Benefícios Obtidos

- **Simplicidade de Uso:** Todo o sistema pode ser orquestrado com apenas uma instância (`plataforma`).
- **Encapsulamento Complexo:** A integração com outros padrões — como o Builder sendo acionado dentro do `TrilhaService` ou os Observers que são ativados no `ProgressoService` — fica totalmente transparente para o Cliente.
- **Alta Coesão e Baixo Acoplamento:** As regras do negócio estão coesas em seus respectivos serviços, mas o acoplamento do cliente ao sistema cai drasticamente, facilitando testes e manutenção do fluxo principal da aplicação.
...
>
> 3. REFACTORING GURU. Facade Pattern. Refactoring Guru, [s.d.]. Disponível em: [https://refactoring.guru/design-patterns/facade](https://refactoring.guru/design-patterns/facade). Acesso em: 21 mai. 2026.
>
> 4. SERRANO, Milene. Arquitetura e Desenho de Software - Aula GoFs Estruturais. [Material de aula em PDF]. Brasília: UnB Gama.

## 4. Histórico de Versões

| Versão | Data       | Descrição                   | Autor            | Revisor          |
|--------|------------|-----------------------------|------------------|------------------|
| 1.0    | 22/05/2026 | Documentação do Padrão Facade  | Vinícius Ribeiro | Arthur Fernandes |
